...
class WithdrawalService {
    ...
    async updateAccountStatus(accountId) {
        try {
            ...
            const driver = await Driver.findOne({ stripeConnectAccountId: accountId });
            ...
            const updates = {
                ...
                stripeOnboardingCompleted: account.details_submitted && account.payouts_enabled,
            };
    ...
    async validateWithdrawalRequest(driverId, amount) {
        ...
        if (amount < this.minimumWithdrawalAmount) {
            throw createError.BadRequest(`Minimum withdrawal amount is $${this.minimumWithdrawalAmount / 100}`);
        }

        // Check maximum amount
        if (amount > this.maximumWithdrawalAmount) {
            throw createError.BadRequest(`Maximum withdrawal amount is $${this.maximumWithdrawalAmount / 100}`);
        }

        // Check available balance
        const availableBalance = await Wallet.calculateAvailableBalance(driverId);
        ...
        if (pendingWithdrawals.length > 0) {
            throw createError.BadRequest('You have pending withdrawals. Please wait for them to complete.');
        }
     ...
     */
    async createWithdrawalRequest(driverId, amount, description = 'Driver earnings withdrawal') {
        try {
            // Validate the request
            const { driver } = await this.validateWithdrawalRequest(driverId, amount);
    ...
    async processWithdrawal(withdrawalId) {
        try {
            const withdrawal = await Wallet.findById(withdrawalId).populate('driverId');
            if (!withdrawal) {
                throw createError.NotFound('Withdrawal not found');
            }

            if (withdrawal.status !== 'pending') {
                throw createError.BadRequest('Withdrawal is not in pending status');
            }
    ...
    async processAutoWithdrawals() {
        try {
            ...
            for (const driver of driversWithAutoWithdrawal) {
                try {
                    const availableBalance = await Wallet.calculateAvailableBalance(driver._id);
                    const threshold = driver.withdrawalSettings.autoWithdrawalThreshold;
                    ...
                    if (availableBalance >= threshold) {
                        ...
                        const processedWithdrawal = await this.processWithdrawal(withdrawal._id);
                    ...
                    console.error(`Error processing auto withdrawal for driver ${driver._id}:`, error);
                    results.push({
                        success: false,
                        driverId: driver._id,
                        error: error.message,
    ...
    async cancelWithdrawal(withdrawalId, reason = 'Cancelled by user') {
        try {
            ...
            if (withdrawal.status !== 'pending') {
                throw createError.BadRequest('Can only cancel pending withdrawals');
            }
